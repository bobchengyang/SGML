function [ class_temp_binary, class_temp, ...
    ...
    optimization_M_MLjournal_subgraph_1sbb2f_time_classification_i( class_test, ...
    feature_train_test, ...
    initial_label, ...
    initial_label_index, ...
    class_train_test, ...
...
while igraphknn>1e-4
    ...
    for time_i=1:run_t
        ...
        if ig_counter==0
            ...
            [M_current_eigenvector,scaled_M,scaled_factors] = optimization_M_scalars(M,n_feature,1,M_current_eigenvector,bins);
        end
    ...
    currentc_obj=x_valid'*L*x_valid;
    disp(['objective after metric learning : ' num2str(currentc_obj)]);
    disp(['error rate after metric learning : ' num2str(error_classifier)]);
    
    igraphknn=norm(currentc_obj-initialc_obj);
    initialc_obj=currentc_obj;
