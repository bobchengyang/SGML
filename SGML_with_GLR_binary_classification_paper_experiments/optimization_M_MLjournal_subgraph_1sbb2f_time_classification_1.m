function [ class_temp_binary, class_temp, ...
    GT_obj_all, obj_all, error_iter,obj_vec,time_vec,error_classifier] = ...
    optimization_M_MLjournal_subgraph_1sbb2f_time_classification_1( class_test, ...
    feature_train_test, ...
    initial_label, ...
    initial_label_index, ...
    class_train_test, ...
...
x_known=class_train_test(initial_label_index);
x_unknown=class_train_test(~initial_label_index);
x_known_idx=find(initial_label_index==1);
x_unknown_idx=find(initial_label_index==0);
gc_length=length(x_known)+1;

x_valid = class_train_test;

for gc_i=1:length(x_unknown)
    feature_idx=[x_known_idx;x_unknown_idx(gc_i)];
    [ L ] = optimization_M_set_L_Mahalanobis_tt( feature_train_test(feature_idx,:), M ); % full observation
    
    %=======Graph classifier starts=======
    cvx_begin
    %variable x(n_sample,1);
    variable x(gc_length,1);
    minimize(x'*L*x)
    subject to
    %x(initial_label_index) == class_train_test(initial_label_index);
    x(1:gc_length-1) == x_known;
    cvx_end
    
    %x_valid = sign(x);
    x_valid(x_unknown_idx(gc_i))=sign(x(gc_length));
    %========Graph classifier ends========
end
