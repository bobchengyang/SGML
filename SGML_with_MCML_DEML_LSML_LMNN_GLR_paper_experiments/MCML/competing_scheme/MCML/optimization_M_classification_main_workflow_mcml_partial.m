function [ class_temp_binary, class_temp, ...
    ...
    optimization_M_classification_main_workflow_mcml_partial( class_test, ...
    feature_train_test, ...
    initial_label, ...
    initial_label_index, ...
    class_train_test, ...
...
for time_i=1:run_t
    ...
    initial_objective = mcml_obj(M, partial_feature, P, partial_sample);
    disp(['initial = ' num2str(initial_objective)]);
    
    M = mcml(M, partial_feature, partial_observation);
    M=M.A;
    ...
    current_objective = mcml_obj(M, partial_feature, P, partial_sample);
