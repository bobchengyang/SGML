function [ class_temp_binary, class_temp, ...
    ...
    optimization_M_classification_main_workflow_dml_partial( class_test, ...
    feature_train_test, ...
    initial_label, ...
    initial_label_index, ...
    class_train_test, ...
...
[w,S,D] = get_S_and_D(partial_feature, partial_observation);
for time_i=1:run_t
    
    M=optimization_M_initialization(n_feature,0);

    initial_objective=dml_obj(M, partial_feature, D);
    
    disp(['initial = ' num2str(initial_objective)]);
    tic;

    [M, ~] = dml_main(partial_feature, S, D, M, w, S_upper, Inf);
