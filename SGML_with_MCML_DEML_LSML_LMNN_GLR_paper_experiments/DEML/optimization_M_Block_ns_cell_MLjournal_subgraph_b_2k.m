function [M,...
    ...
    min_objective,...
    bins,...
    invalid_result] = optimization_M_Block_ns_cell_MLjournal_subgraph_b_2k(current_node,league_vec,league_vec_temp,flip_number,...
    Ms_off_diagonal,...
    scaled_factors_v,...
    scaled_factors_h,...
    feature_N,...
...
while tol_offdia>1e-5
    ...
    if isequal(league_vec,league_vec_temp)==1
        ...
        while objective_current_step_size>objective_previous
            if gamma<1e-8
                t_M21 = M_previous;
                t_M21=t_M21.*zero_mask;
                M_lc(BCD,remaining_idx)=t_M21(1:feature_N-1);
        ...
        while objective_current_step_size<objective_previous
            objective_previous=objective_current_step_size;
            if gamma<1e-8
                t_M21 = M_previous;
                t_M21=t_M21.*zero_mask;
                M_lc(BCD,remaining_idx)=t_M21(1:feature_N-1);
...
%     end
    ...
    M_updated(abs(M_updated)<1e-4)=0;
